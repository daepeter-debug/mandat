...
export default function PoliticalNewsFeed({compact=false,onOpen}:{compact?:boolean;onOpen?:()=>void}){
  ...
  return <section className={compact?'news-digest':'news-page'} aria-labelledby={uid}>
    ...
    {!compact&&<div className="news-filters"><div role="group" aria-label="Obdobie správ">{[['week','Tento týždeň'],['today','Dnes'],['all','Celý výber']].map(([id,label])=><button key={id} aria-pressed={period===id} onClick={()=>setPeriod(id)}>{label}</button>)}</div><label>Téma<select value={category} onChange={e=>setCategory(e.target.value)}><option value="all">Všetky témy</option>{['Vláda','Parlament','Prieskumy'].map(c=><option key={c}>{c}</option>)}</select></label></div>}
