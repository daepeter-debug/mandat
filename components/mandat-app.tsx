...
import { canMorph, morphTo } from "@/lib/morph";
...
import { Fold, MobileCollapse } from "@/components/mobile-fold";
...
export default function MandatApp() {
  ...
  const openParty = (p:Party|null, logo?:Element|null) => {
    if(p&&canMorph(logo)) morphTo(logo,"party-logo","vt-party",()=>{setPartyMorph(true);setParty(p);});
    else {setPartyMorph(false);setParty(p);}
  };
  ...
  return <div className="site-shell editorial-shell with-party-rail">
    ...
    <PartyRail selected={ui.party} onSelect={(p,logo)=>openParty(ui.party===p.id?null:p,logo)} onMethod={()=>{update({party:null,view:"method"},true);window.scrollTo({top:0,behavior:"instant"});}}/>
