...
import { scenarioFromPoll } from "@/lib/parliament";
...
export function MandatMagazine({poll,onAgency,onNavigate}:{poll:Poll;onAgency:(a:string)=>void;onNavigate:(v:string)=>void}) {
  ...
  return <div className="magazine">
    ...
    <section className="mag-pulse" aria-labelledby="pulse-title"><div className="mag-section-head"><div><h2 id="pulse-title">Jednotlivé merania pod lupou.</h2><p>Agregát je hlavný pohľad. Tu si môžete skontrolovať každú agentúru osobitne.</p></div><AgencyPicker value={poll.agency} onChange={onAgency}/></div><div className="mag-poll-meta"><b>{poll.agency} · {poll.month.toLowerCase()} {poll.end.slice(0,4)}</b><span>Zber {date(poll.start)} – {date(poll.end)} · n = {poll.sample?.toLocaleString("sk-SK")??"neuvedené"}</span></div>
