...
export default function Home() {
  ...
  return <div className="site-shell editorial-shell with-party-rail">
    ...
    <Tabs value={view} onValueChange={changeView} activationMode="manual" className="page-tabs">
      ...
      <main id="main">
        ...
        <TabsContent value="polls">
          <section className="intro"><div><h1>Každé meranie. <em>V kontexte.</em></h1><p className="intro-description">Kto sa pýtal, kedy a koho. Výsledky spolu s metodikou a pôvodným zdrojom.</p></div></section>
        ...
        <TabsContent value="parties">
          <section className="intro"><div><h1>Strany. <em>Bez skratiek v úsudku.</em></h1><p className="intro-description">Zameranie, ľudia a programy. Spoznajte strany za číslami a overte si, odkiaľ informácie pochádzajú.</p></div></section>
          ...
          <h2 className="sr-only">Zoznam politických subjektov</h2><div className="party-grid">{visibleParties.map(p=><article className="party-card" key={p.id}><div className="party-card-top"><span className="party-monogram"><i style={{background:p.color}} aria-hidden="true"/>{p.short}</span><ChevronRight size={18} aria-hidden="true"/></div><button className="party-card-action" onClick={()=>setParty(p)}><h3>{p.name}</h3><span>Pozrieť profil <ArrowRight size={16}/></span></button><PartyTags partyId={p.id}/>{partyProfiles[p.id]&&<p className="party-card-summary">{partyProfiles[p.id].summary}</p>}<div className="party-card-stat"><div><span>{current.agency} · {current.month.toLowerCase()} 2026</span><strong>{current.values[p.id]===undefined?"—":`${fmt(current.values[p.id])} %`}</strong></div><Source poll={current}/></div>{current.values[p.id]===undefined&&<p className="missing-caption">Presná hodnota zatiaľ nie je prepísaná.</p>}</article>)}</div>
        ...
        <TabsContent value="method">
          <section className="intro"><div><h1>Čísla potrebujú <em>súvislosti.</em></h1><p className="intro-description">Čo v našich dátach nájdete, ako s nimi pracujeme a kde sú ich hranice.</p></div></section>
    ...
    <footer><button className="brand small" onClick={()=>changeView("overview")}>mandát.</button><p>Nezávislý hobby projekt. Bez reklamy.</p><button className="text-button" onClick={()=>changeView("method")}>Metodika a zdroje <ArrowRight size={13}/></button><span>Dáta skontrolované {verified}</span></footer>
